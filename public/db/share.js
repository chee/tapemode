...
export async function init(sab) {
	...
	return new Promise((yay, boo) => {
		...
		open.onupgradeneeded = (/** @type {IDBVersionChangeEvent} */ event) => {
			...
			db = open.result
			let store
			if (db.objectStoreNames.contains("tape")) {
				store = open.transaction.objectStore("tape")
			} else {
				store = db.createObjectStore("tape", {
					autoIncrement: false
				})
			}
...
export function save(id) {
	...
	let trans = db.transaction("tape", "readwrite", {
		durability: "relaxed"
	})
	let store = trans.objectStore("tape")
